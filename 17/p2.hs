...
getCombo (registers, instructions, pointer)
  ...
  | val == 6 = get registers 'C'
  where
    val = instructions !! (pointer + 1)

adv :: Operation
adv state = (Nothing, (newregisters, instructions, pointer + 2))
  where
...
bxl :: Operation
bxl (registers, instructions, pointer) = (Nothing, (newregisters, instructions, pointer + 2))
  where
    op1 = get registers 'B'
...
bst :: Operation
bst state = (Nothing, (newregisters, instructions, pointer + 2))
  where
    (registers, instructions, pointer) = state
    op = getCombo state
    res = mod op 8
...
jnz :: Operation
jnz (registers, instructions, pointer) = (Nothing, (registers, instructions, newpointers))
  where
    ra = get registers 'A'
...
bxc :: Operation
bxc (registers, instructions, pointer) = (Nothing, (newregisters, instructions, pointer + 2))
  where
    b = get registers 'B'
    c = get registers 'C'
    res = xor b c
...
out :: Operation
out state = (Just result, (registers, instructions, pointer + 2))
  where
    (registers, instructions, pointer) = state
    op = getCombo state
    result = mod op 8
...
bdv :: Operation
bdv state = (Nothing, (newregisters, instructions, pointer + 2))
  where
    (registers, instructions, pointer) = state
    num = get registers 'A'
    denom = 2 ^ (getCombo state)
...
cdv :: Operation
cdv state = (Nothing, (newregisters, instructions, pointer + 2))
  where
    (registers, instructions, pointer) = state
    num = get registers 'A'
    denom = 2 ^ (getCombo state)
...
doNextOp state = if isJust op then Just $ (fromJust op) state else Nothing
  where
