...
makeInitialState width start = (scoremap, scores, width)
  where
...
getNextVisit scoremap
  ...
  | otherwise = (Just (head vals), newmap)
  where
...
updateScoremap scoremap location old new = newscoremap
  where
...
doScoreUpdate ms [] = ms
doScoreUpdate (scoremap, scores, width) (first : rest) = doScoreUpdate (nsm, nscores, width) rest
  where
...
getNeighbors width start = forward : side
  where
    area = width * width
...
hasWall map width idx = elem mapidx map
  where
    mapidx = mod idx (width * width)

getNeighborCostUpdates :: Map -> Scores -> Int -> Int -> [(Int, Int)]
getNeighborCostUpdates mp scores width index = ns
  where
...
filterNeighborCostUpdates scores index (first : rest)
  ...
  | otherwise = filterNeighborCostUpdates scores index rest
  where
    (newscore, index) = first
    curscore = IM.lookup index scores

doUpdate :: Map -> MapState -> MapState
doUpdate map state = if isNothing next || nextidx < 0 then state else doUpdate map newstate
  where
...
getLowestFeeders mp scores idx width = feeders
  where
...
getFeedTiles idx width = idxs ++ turnsInTile
  where
    area = width * width
    tindx = mod idx area
