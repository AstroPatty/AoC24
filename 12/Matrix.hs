...
setVal val m (rowi,coli) = if isNothing $ getVal m (rowi, coli) then m else newm where
...
setAllInRow vals idxs val = zipWith (replaceInRow) vals [0..] where
    replaceInRow = \v ci -> if elem ci idxs then val else v

findVal :: Eq t => Matrix t -> t -> Maybe Index
findVal [] _ = Nothing
findVal matrix value = if length idxs > 0 then Just $ head idxs else Nothing where    
